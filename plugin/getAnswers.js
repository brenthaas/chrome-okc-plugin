_OKCP.getAnswers = function(list) {
  ...
  var recentProfiles = localStorage.okcpRecentProfiles
    ...
    : {
        _ATTENTION:
          "This is just temporary caching to avoid hitting the server a million " +
          "times. Notice there's an expires time built in for each key."
      };
    ...
    for (var category in list) {
      ...
      for (var i = 0; i < categoryQuestionList.length; i++) {
        ...
        if (_OKCP.onOwnProfile) {
          ...
          theirAnswer = questionElem
            .find(".my_answer input:checked + label")
            .text()
            .trim();
          theirNote = questionElem
            .find(".explanation textarea")
            .text()
            .trim();
        } else {
          theirAnswer = questionElem
            .find("#answer_target_" + num)
            .text()
            .trim();
          if (theirAnswer === "") continue; //if the answer elem doesn't exist, continue
          theirNote = questionElem
            .find("#note_target_" + num)
            .text()
            .trim();
          yourAnswer = questionElem
            .find("#answer_viewer_" + num)
            .text()
            .trim();
          yourNote = questionElem
            .find("#note_viewer_" + num)
            .text()
            .trim();
        }
  ...
  function loadProfileAnswers() {
    ...
    $("#footer").append(
      '<a class="page-results-link" href="#page-results">Show question results</a>'
    );
  ...
  function updateQuestionPath(pageNum) {
    if (
      ...
      questionPath =
        ...
        _OKCP.profileName +
        "/questions?n=2&low=" +
        (questionPageNum * 10 + 1) +
        "&" +
        questionFilterParameter;
      if (_OKCP.questionFetchingMethod === "mobile_app")
        questionPath += "&leanmode=1";
      questionPageNum++;
    }
  }
  ...
  function areWeDone(fromCached) {
    ...
    for (var category in responseCount) {
      ...
      var matchRatioHtmlValue =
        '<span class="integer">' +
        numeratorArr[0] +
        '</span><span class="point">.</span><span class="decimal">' +
        (numeratorArr[1] || "0") +
        '</span><span class="slash">/</span><span class="integer">' +
        denominatorArr[0] +
        '</span><span class="point">.</span><span class="decimal">' +
        (denominatorArr[1] || "0") +
        "</span>";
      $(
        '<li class="match-ratio ' +
          matchClass +
          '" category="' +
          category +
          '"><span class="match-ratio-progressbar ' +
          matchClass +
          '" style="width:' +
          (Math.round(countArr[0] / countArr[1] * 93) + 7) +
          '%"></span><span class="match-ratio-category">' +
          categoryReadable +
          '</span><span class="match-ratio-value">' +
          matchRatioHtmlValue +
          "</span></li>"
      )
      ...
      for (var i = 0; i < questionList.length; i++) {
        ...
        if (question.category === category) {
          ...
          $(".question-detail-" + question.category).append(
            '<li class="match ' +
              matchClass +
              '"><ul>' +
              '<li class="question qid-' +
              question.qid +
              '">' +
              question.question +
              "</li>" +
              '<li class="answer">' +
              question.theirAnswer +
              "</li>" +
