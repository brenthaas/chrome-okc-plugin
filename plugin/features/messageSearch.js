_OKCP.messageSearch = function() {
  ...
  var indexMessagesNow =
    _OKCP.storage("messagesIndexed") + twoWeeksInMilliseconds <
    new Date().getTime(); //reindex every two weeks
  ...
  if (indexMessagesNow) {
    ...
    _OKCP.storage(
      "messagesIndexed",
      new Date().getTime() - twoWeeksInMilliseconds + 60 * 5
    );
    //console.log(_OKCP.storage('messagesIndexed'));

    //console.log('Indexing messages');
    $('<iframe src="//www.okcupid.com/messages"></iframe>')
      .appendTo("body")
      .load(function() {
        _OKCP.indexMessages($(this.contentDocument), this.contentWindow);
      });
    $('<iframe src="//www.okcupid.com/messages?folder=2"></iframe>')
      .appendTo("body")
      .load(function() {
        _OKCP.indexMessages($(this.contentDocument), this.contentWindow);
      });
...
_OKCP.addMessageLinkUI = function() {
  ...
  messageLinkUI = $(
    '<a href="#" class="flatbutton white UI-change-btn btn-previous-messages">View Previous Messages</a>'
  ).hide();
  $(".action_options")
    ...
    if (prevThreads !== undefined) {
      for (var i = 0; i < prevThreads.length; i++) {
        ...
        for (var j = 0; j < threads.length; j++) {
          if (threadToAdd.uninterested === threads[j].uninterested) {
...
_OKCP.loadUrlsIntoElem = function(urls, selector, callback) {
  ...
  function requestComplete(e, status) {
    ...
    if (numCompleteRequests === urls.length) callback(pageResultsDiv);
  }
